...
public class PaymentManager implements IPaymentManager, IPaymentManagerNotify {
	...
	public boolean tenderChange() throws RuntimeException, NoCashAvailableException {
		...
		for (int i = this.machine.banknoteDenominations.length - 1; i >= 0; i--) {
			BigDecimal denomination = this.machine.banknoteDenominations[i];
		...
		for (int i = 0; i <= this.machine.coinDenominations.size() - 1; i++) {
